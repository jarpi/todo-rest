...
public class MongoDBImpl { 
	...
	public MongoDBImpl(String ip) throws UnknownHostException {
		if (mMongoClient == null) {
			try {
				mMongoClient = new MongoClient(ip, 27017); 
				mDb = mMongoClient.getDB("tictactoe");
	...
	public DBObject getDocumentById(String collection, String id) throws Exception { 
		...
		if (mDb.collectionExists(collection)) {
			DBCollection coll = mDb.getCollection(collection); 
